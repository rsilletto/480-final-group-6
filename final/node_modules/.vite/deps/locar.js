...
var E = class {
  ...
  stopGps() {
    return this._watchPositionId !== null ? (navigator.geolocation.clearWatch(this._watchPositionId), this._watchPositionId = null, true) : false;
  }
   ...
   */
  _haversineDist(e, t) {
    const i = MathUtils.degToRad(t.longitude - e.longitude), s = MathUtils.degToRad(t.latitude - e.latitude), a = Math.sin(s / 2) * Math.sin(s / 2) + Math.cos(MathUtils.degToRad(e.latitude)) * Math.cos(MathUtils.degToRad(t.latitude)) * (Math.sin(i / 2) * Math.sin(i / 2));
    return 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a)) * 6371e3;
  }
};
var _ = class {
  constructor(e, t, i) {
    this.renderer = e, this.renderer.autoClear = false, this.sceneWebcam = new Scene();
    let s;
    t === void 0 ? (s = document.createElement("video"), s.setAttribute("autoplay", true), s.setAttribute("playsinline", true), s.style.display = "none", document.body.appendChild(s)) : s = document.querySelector(t), this.geom = new PlaneGeometry(), this.texture = new VideoTexture(s), this.material = new MeshBasicMaterial({ map: this.texture });
    const a = new Mesh(this.geom, this.material);
    if (this.sceneWebcam.add(a), this.cameraWebcam = new OrthographicCamera(
      -0.5,
      0.5,
      0.5,
      -0.5,
...
var M = new Quaternion(-Math.sqrt(0.5), 0, 0, Math.sqrt(0.5));
var f = { type: "change" };
var P = class extends EventDispatcher {
  constructor(e) {
    ...
    const t = this, i = 1e-6, s = new Quaternion();
    this.object = e, this.object.rotation.reorder("YXZ"), this.enabled = true, this.deviceOrientation = {}, this.screenOrientation = 0, this.alphaOffset = 0, this.deviceOrientationEventName = "ondeviceorientationabsolute" in window ? "deviceorientationabsolute" : "deviceorientation";
    const a = function(o) {
    ...
    this.connect = function() {
      ...
      }) : (window.addEventListener("orientationchange", c), window.addEventListener(t.deviceOrientationEventName, a)), t.enabled = true;
    }, this.disconnect = function() {
      window.removeEventListener("orientationchange", c), window.removeEventListener(t.deviceOrientationEventName, a), t.enabled = false;
    }, this.update = function() {
      if (t.enabled === false) return;
      const o = t.deviceOrientation;
      if (o) {
        const h = o.alpha ? MathUtils.degToRad(o.alpha) + t.alphaOffset : 0, l = o.beta ? MathUtils.degToRad(o.beta) : 0, u = o.gamma ? MathUtils.degToRad(o.gamma) : 0, m = t.screenOrientation ? MathUtils.degToRad(t.screenOrientation) : 0;
...
var y = class {
  constructor(e) {
    this.raycaster = new Raycaster(), this.normalisedMousePosition = new Vector2(null, null), e.domElement.addEventListener("click", (t) => {
  ...
  raycast(e, t) {
    if (this.normalisedMousePosition.x !== null && this.normalisedMousePosition.y !== null) {
      ...
      const i = this.raycaster.intersectObjects(t.children, false);
...
function b(r = {}) {
  const e = new Scene(), t = new PerspectiveCamera(
    r.fov || 60,
